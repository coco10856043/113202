...
def afterlogin_home():
    ...
    for story in storys:
        if story[4] != user_id: 
            ...
            # 查詢資料表
            cursor = conn.cursor()
            cursor.execute("select name from kind where kind_id = %s", (str(story[2]),))
            story_kind = cursor.fetchone()

            story_user_id = str(story[4])
            # 取得資料庫連線
            conn = db.get_connection()
            # 查詢資料表
            cursor = conn.cursor()
            cursor.execute("select name, avatar from user_information where user_id = %s", (story_user_id,))
            user = cursor.fetchone()
            if len(story[5]) > 40:
                story_detail = story[5][:40] + '...'
            else:
                story_detail = story[5]

            # 取得資料庫連線
            conn = db.get_connection()
            # 查詢資料表
            cursor = conn.cursor()
            cursor.execute("select * from collect where story_id = %s", (str(story[0]),))
            collects = cursor.fetchall()
            num_collects = len(collects)
            follow_state = '未追蹤'

            conn = db.get_connection()
            # 查詢資料表
            cursor = conn.cursor()
            cursor.execute("select * from follow where user_id = %s and follow_user = %s", (user_id, story_user_id,))
            find_follow = cursor.fetchone()
...
def add_follow():
    ...
    try:
        ...
        # 插入追蹤記錄
        cursor.execute("INSERT INTO follow (user_id, follow_user) VALUES (%s, %s)", (user_id, follow_user_id[0]))
        conn.commit()
        conn.close()
...
def myStory():
    ...
    return render_template('myStory.html', username = username, stories=stories_list)
...
def searchmystory():
    ...
    # 查詢資料表
    cursor = conn.cursor()
    cursor.execute("select * from story where user_id = %s and title LIKE %s", (user_id, f"%{search_mystory}%",))
    storys = cursor.fetchall()
...
def collection():
    ...
    # 查詢資料表
    cursor = conn.cursor()
    cursor.execute("select * from collect where user_id = %s", (user_id, ))
    storys = cursor.fetchall()
    ...
    for story in storys:
        ...
        if len(story_information[5]) > 40:
            story_detail = story_information[5][:40] + '...'
        else:
            story_detail = story_information[5]

        user = str(story_information[4])
        # 取得資料庫連線
        conn = db.get_connection()
        # 查詢資料表
        cursor = conn.cursor()
        cursor.execute("select name from user_information where user_id = %s", (user,))
        story_username = cursor.fetchone()

        stories_list.append({
            "image": "/static/image/story_cover/" + story_information[6],
            "title": story_information[1],
            "date": story[2],
            "user": story_username[0],
            "summary": story_detail,
            "link": '/story_details?story_id='+str(story_information[0])
        })
...
def searchmycollection():
    ...
    for story in storys:
        ...
        if search_mycollection in story_information[1]:
            if len(story_information[5]) > 40:
                story_detail = story_information[5][:40] + '...'
            else:
                story_detail = story_information[5]
...
def story_details():
    ...
    # 查詢資料表
    cursor = conn.cursor()
    cursor.execute("select * from story where story_id = %s", (story_id,)) # 可得書名、封面、風格id、大綱、作者id
    story_title = cursor.fetchall()
    
    # 取得資料庫連線
    conn = db.get_connection()
    # 查詢資料表
    cursor = conn.cursor()
    cursor.execute("select name from user_information where user_id = %s", (story_title[0][4],)) # 可得作者名稱
    story_user = cursor.fetchall()

    # 取得資料庫連線
    conn = db.get_connection()
    # 查詢資料表
    cursor = conn.cursor()
    cursor.execute("select name from kind where kind_id = %s", (story_title[0][2],)) # 可得風格名稱
    ...
    # 查詢資料表
    cursor = conn.cursor()
    cursor.execute("select picture from pages where story_id = %s order by page", (story_title[0][0],)) # 可得故事各頁圖片
    pages_picture = cursor.fetchall()
    ...
    # 查詢資料表
    cursor = conn.cursor()
    cursor.execute("select * from follow where user_id = %s and follow_user = %s", (user_id, story_title[0][4],))
    find_follow = cursor.fetchone()
    ...
    # 查詢資料表
    cursor = conn.cursor()
    cursor.execute("select * from story where user_id = %s", (story_title[0][4],))
    stories = cursor.fetchall()
    total_stories = len(stories) # 計算作者創作的數量

    conn = db.get_connection()
    # 查詢資料表
    cursor = conn.cursor()
    cursor.execute("select * from collect where story_id = %s", (story_id,))
    collections = cursor.fetchall()
    total_collections = len(collections) # 計算本書被收藏數

    conn = db.get_connection()
    # 查詢資料表
    cursor = conn.cursor()
    cursor.execute("select * from follow where follow_user = %s", (story_title[0][4],))
    follow_users = cursor.fetchall()
    total_follow_users = len(follow_users) # 計算粉絲數

    detail_list = []
    detail_list.append({
                "title": story_title[0][1],
                "detail": story_title[0][5],
                "kind": kind[0][0],
                "user": story_user[0][0],
                "follow": follow_state,
                ...
                "picture_cover": "/static/image/story_cover/" + story_title[0][6],
                "picture_page1": "/static/image/page/" + pages_picture[0][0],
                "picture_page2": "/static/image/page/" + pages_picture[1][0],
                "picture_page3": "/static/image/page/" + pages_picture[2][0],
                "picture_page4": "/static/image/page/" + pages_picture[3][0],
                "picture_page5": "/static/image/page/" + pages_picture[4][0]
            })
...
def change():
    try:
        ...
        try:
            ...
            return render_template('forget.html', error_message="密碼修改失敗，查無此使用者2")

    except Exception as e:
            print(f"An error occurred: {e}")
            return render_template('forget.html', error_message="密碼修改失敗，查無此使用者1")
...
def generate_images():
    try:
        ...
        print(message)
        # 更新 prompt，使风格更可爱
        prompt = "故事風格為" + story_kind + "故事段落為" + message + "，生成的圖片不准出現任何的文字(包含英文和中文)，只要是圖片就好，而且一定要保證圖片風格"
...
@app.route('/static/image/storys/<path:filename>')
...
def save_story():
    try:
        ...
        if not all([message1, image_filename1, message2, image_filename2,
            ...
            return render_template('edit.html', error_message="儲存失敗，請檢查所有故事內容是否填寫完整或圖片是否皆生成完成。", story_title = story_title, story_kind = story_kind, array_story = array_story)
...
# # 設定OpenAI API Key
# openai.api_key = os.getenv("sk-proj-RmZpsV6UyStRaqrVD6x2T3BlbkFJ3b47K7Oo7YCKm8X4zrMr")
