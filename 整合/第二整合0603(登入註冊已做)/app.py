...
def login():
    try:
        ...
        #將資料加入資料表
        cursor = conn.cursor()
        cursor.execute("select * from user_information where user_id = %s and password = CAST(%s AS VARCHAR)", (title, hash_password, ))
...
# client = OpenAI(api_key=os.environ.get("OPENAI_API_KEY", "OpenAI APIKEY"))
# # 設置你的 OpenAI API 密鑰
# 
