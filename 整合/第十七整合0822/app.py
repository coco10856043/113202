...
def creation():
    try:
        ...
        run = client.beta.threads.runs.create(
            thread_id=thread.id,
            assistant_id='assistant_id', # 助理名稱：story1
        )
        ...
        return render_template('creation.html', error_message="創作故事失敗")

# 生成圖片
#-----------------------
save_folder = os.path.join(os.getcwd(), 'static', 'image', 'story_select')
...
def generate_images():
    try:
        ...
        print(message)
        # 更新 prompt，調整為可愛的風格
        prompt = "故事風格為" + story_kind + "故事段落為" + message + "，重要的是圖片中不可以有任何的文字元素，並且圖片中的角色應該在所有生成的圖片中保持一致的外貌特徵和風格，確保小孩能夠辨識出同一個角色。圖片的風格要統一，一定要讓小孩看得懂。"
...
def save_story():
    try:
        ...
        # 移動封面圖片
        source = os.path.join(os.getcwd(), 'static', 'image', 'story_select')
        destination_cover = os.path.join(os.getcwd(), 'static', 'image', 'story_cover')
        destination_page = os.path.join(os.getcwd(), 'static', 'image', 'page')
        ...
        return render_template('creation.html', success_message="故事創作成功，可到歷史創作欣賞作品。")
