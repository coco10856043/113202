...
ALLOWED_EXTENSIONS = {'jpg', 'png', 'jpeg'}  # 允許上傳的文件類型
...
def do_register():
    ...
    try:
        ...
        password = request.form.get('password0')
        avatar = request.files.get('avatar')  # 取得上傳的頭像文件

        # 驗證表單參數是否齊全
        if not (title and name and password):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO user_information (user_id, name, password, avatar) VALUES (%s, %s, %s, %s)",
                (title, name, hash_password, filename)
            )
            conn.commit()
            conn.close()
...
def home():
    ...
    try:
        ...
        if user and bcrypt.checkpw(password.encode('utf-8'), user[2].encode('utf-8')):
            user_id = user[0]
            username = user[1]
            avatar = "static/avatar/" + user[3]
            
            session['user_id'] = user_id
            session['username'] = username
            session['lookavatar'] = avatar
...
def afterlogin_home():
    ...
    conn.close()  # 結束資料庫連線
    return render_template('home.html', username=username, lookavatar=lookavatar, stories=stories_list)
...
def story_details():
    ...
    query = "SELECT s.story_id, s.title, s.story_detail, s.story_picture, s.user_id AS author_id, k.name AS kind_name, p.picture AS page_picture, s.state FROM story s JOIN kind k ON s.kind_id = k.kind_id LEFT JOIN pages p ON s.story_id = p.story_id WHERE s.story_id = %s"
    ...
    return render_template('story-details.html', username=username, lookavatar=lookavatar, details=detail_list[0])
...
def myStory():
    ...
    return render_template('myStory.html', username=username, lookavatar=lookavatar, htmltitle = htmltitle, stories=stories_list)
...
def add_open():
    ...
    try:
        ...
        cursor.execute("UPDATE story SET state=%s WHERE story_id=%s", ('true', story_ID))
...
# 取消公開動作
#-----------------------
@app.route('/delete_open', methods=['POST'])
def delete_open():
    data = request.get_json()  # 從請求中取得 author_name 參數
    story_ID = data.get('story_ID')
    ...
    try:
        ...
        cursor.execute("UPDATE story SET state=%s WHERE story_id=%s", ('false', story_ID))
...
def collection():
    ...
    return render_template('collection.html', username=username, lookavatar=lookavatar, stories=stories_list)
...
def follow():
    ...
    return render_template('follow.html', username=username, lookavatar=lookavatar, users_information=users_info_list)
...
def searchuser():
    ...
    # 渲染模板
    return render_template('follow.html', username=username, lookavatar=lookavatar, users_information=users_information)
...
def user_detail():
    ...
    return render_template('followStory.html', username=username, search_followname=search_followname, lookavatar=lookavatar, htmltitle=htmltitle, stories=stories_list)

# 搜尋追蹤者的故事動作
#-----------------------
@app.route('/searchfollowstory', methods=['GET'])
def searchfollowstory():
    search_followstory = request.args.get('search_followstory')
    search_followname = request.args.get('search_followname')
    # 初始化 htmltitle 以确保它在函数的所有路径中都被赋值
    htmltitle = f"{search_followname}的創作" if search_followname else "用户的創作"
    ...
    cursor = conn.cursor()
    # 查詢資料表：找出符合搜尋條件的故事
    cursor.execute("SELECT user_id FROM user_information WHERE name = %s", (search_followname, ))
    followuser = cursor.fetchone()
    followuser_id = str(followuser[0])
    # 查詢資料表：找出符合搜尋條件的故事
    cursor.execute("SELECT story_id, title, story_detail, story_picture FROM story WHERE user_id = %s AND title LIKE %s", (followuser_id, f"%{search_followstory}%"))
    stories = cursor.fetchall()
...
def creation_story():
    username = session['username']
    lookavatar = session['lookavatar']
    return render_template('creation.html', username = username, lookavatar = lookavatar)
...
def create_name():
    ...
    kind = data.get('kind')
    question = f"請根據風格 '{kind}' 生成一個故事標題。"
    ...
    run = client.beta.threads.runs.create(
        thread_id=thread.id,
        assistant_id='assistant_id', # 助理名稱：story_name
    )
    ...
    response_text = response_text.strip()  # Remove any leading or trailing whitespace
    print(response_text)

    return jsonify({'generated_title': response_text})

# 隨機生成故事大綱
#-----------------------
@app.route('/create_detail', methods=['POST'])
def create_detail():
    data = request.get_json()
    kind = data.get('kind')
    title = data.get('title')
    question = f"請根據風格 '{kind}' 和標題 '{title}' 生成一個故事大綱。"
    ...
    run = client.beta.threads.runs.create(
        thread_id=thread.id,
        assistant_id='assistant_id', # 助理名稱：story_detail
    )
    ...
    return jsonify({'generated_detail': response_text})
...
def creation():
    ...
    needs_retry = any(len(i) > 1 and "第" in i[1] and "段" in i[1] for i in array_story)
    
    if needs_retry:
        # Redirect with the same form data
        return redirect(url_for('creation', error_message="故事生成失敗，請重新嘗試"))

    return render_template('edit.html', lookavatar=lookavatar, story_title=title, story_kind=kind, story_detail=story_detail, array_story=array_story)

# 故事2次編輯
#-----------------------
@app.route('/edit2_story')
def edit2_story():
    story_id = request.args.get('story_id')
    lookavatar = session['lookavatar']
    # 取得資料庫連線
    ...
    story_detail = cursor.fetchone()

    # 獲取 故事類型
    cursor.execute("SELECT name FROM kind WHERE kind_id = %s", (story_detail[2],))
    story_kind = cursor.fetchone()

    # 獲取 故事內容
    cursor.execute("SELECT * FROM pages WHERE story_id = %s order by page", (story_id,))
    story_pages = cursor.fetchall()

    storylist_detail = []

    storylist_detail.append({
        'story_id': story_id,
        'title': story_detail[1],
        'kind':story_kind[0],
        'state':story_detail[3],
        'story_detail':story_detail[5],
        'story_cover': "static/image/story_cover/" + story_detail[6],
        'story_page1':story_pages[0][2],
        'story_picture1': "static/image/page/" + story_pages[0][3],
        'story_page2':story_pages[1][2],
        'story_picture2': "static/image/page/" + story_pages[1][3],
        'story_page3':story_pages[2][2],
        'story_picture3': "static/image/page/" + story_pages[2][3],
        'story_page4':story_pages[3][2],
        'story_picture4': "static/image/page/" + story_pages[3][3],
        'story_page5':story_pages[4][2],
        'story_picture5': "static/image/page/" + story_pages[4][3],
    })
    return render_template('edit2.html', lookavatar=lookavatar, storypage = storylist_detail[0])
...
def generate_images():
    try:
        ...
        print(message)
        # 更新 prompt，調整為可愛的風格
        # prompt = "故事風格為" + story_kind + "故事段落為" + message + "，請適合作為故事場景，確保畫中沒有任何文字，還有式小孩看得懂且簡單的"
        # prompt = f"生成一張純圖片。故事風格為{story_kind}，故事段落為：{message}。請確保圖片內不包含任何文字，只展示故事場景和角色。"
        prompt = "Create child-friendly {story_kind} scene based on: {message} . No text, letters, or words, just a visual background for a children's storybook."
...
def save_story():
    try:
        ...
        return render_template('creation.html',
                               lookavatar=lookavatar,
                               success_message="儲存成功，可到歷史創作瀏覽故事")

    except Exception as e:
        print(f"Error: {e}")
        return render_template('edit.html',
                               lookavatar=session.get('lookavatar'),
                               error_message="儲存失敗，請檢查所有故事內容是否填寫完整或圖片是否皆生成完成。",
                               array_story=[['1', message1], ['2', message2], ['3', message3], ['4', message4], ['5', message5]])
...
def update_story():
    try:
        lookavatar = session['lookavatar']
        # 取得參數
        story_id = request.form.get('story_id') # 故事名稱
        story_title = request.form.get('story_title') # 故事名稱
        story_kind = request.form.get('story_kind') # 故事風格
        openstate = request.form.get('isPublic', 'false') # 公開狀態
        story_detail = request.form.get('message0') # 故事大綱
        image_filename_cover = request.form.get('image-filenames0') #故事封面
        message1 = request.form.get('message1') # 故事第一段內容
        image_filename1 = request.form.get('image-filenames1') # 故事第一段圖片
        message2 = request.form.get('message2') # 故事第二段內容
        image_filename2 = request.form.get('image-filenames2') # 故事第二段圖片
        message3 = request.form.get('message3') # 故事第三段內容
        image_filename3 = request.form.get('image-filenames3') # 故事第三段圖片
        message4 = request.form.get('message4') # 故事第四段內容
        image_filename4 = request.form.get('image-filenames4') # 故事第四段圖片
        message5 = request.form.get('message5') # 故事第五段內容
        image_filename5 = request.form.get('image-filenames5') # 故事第五段圖片

        # 检查是否有任何必填值为空，并打印出具体的缺失字段
        required_fields = {
            'story_title': story_title,
            'story_kind': story_kind,
            'story_detail': story_detail,
            'image_filename_cover': image_filename_cover,
            'message1': message1,
            ...
            'message4': message4,
            'image_filename4': image_filename4,
            'message5': message5,
            'image_filename5': image_filename5
        }

        missing_fields = {k: v for k, v in required_fields.items() if v is None or v.strip() == ''}
        if missing_fields:
            ...
            storypage.append({
                ...
                'state': openstate,
                'story_detail': story_detail,
                'story_cover': image_filename_cover,
                'story_page1': message1,
                'story_picture1': image_filename1,
                'story_page2': message2,
                'story_picture2': image_filename2,
                'story_page3': message3,
                'story_picture3': image_filename3,
                'story_page4': message4,
                'story_picture4': image_filename4,
                'story_page5': message5,
                'story_picture5': image_filename5,
            })
            return render_template('edit2.html',
                                   lookavatar=lookavatar,
                                   error_message="更新失敗，請檢查所有故事內容是否填寫完整或圖片是否皆生成完成。",
                                   storypage=storypage[0])
        ...
        if image_filename_cover:
            source_path_cover = os.path.join(source, image_filename_cover)
            destination_path_cover = os.path.join(destination_cover, image_filename_cover)
            if os.path.isfile(source_path_cover) and not os.path.isfile(destination_path_cover):
                shutil.move(source_path_cover, destination_path_cover)
        ...
        # 更新故事信息
        cursor.execute("UPDATE story SET state=%s, story_detail=%s, story_picture=%s WHERE story_id=%s", (openstate, story_detail, os.path.basename(image_filename_cover), story_id))

        # 更新每一段內容和圖片
        for i in range(1, 6):
            message = request.form.get(f'message{i}')
            image_filename = request.form.get(f'image-filenames{i}')
            if message and image_filename:
                cursor.execute("UPDATE pages SET content=%s, picture=%s WHERE story_id=%s AND page=%s", (message, os.path.basename(image_filename), story_id, i))
        ...
        return render_template('creation.html',
                               lookavatar=lookavatar,
                               success_message="更新成功，可到歷史創作瀏覽故事")
...
def user_profile():
    lookavatar = session['lookavatar']
    return render_template('user-profile.html', lookavatar=lookavatar)
...
def update_userinformation():
    try:
        ...
        if not user_id:
            return render_template('user-profile.html', lookavatar=session.get('lookavatar'), error_message="用戶未登入")
        ...
        if not user:
            conn.close()
            return render_template('user-profile.html', lookavatar=session.get('lookavatar'), error_message="用戶不存在")
        ...
        if name:
            ...
            if existing_user and existing_user[0] != user_id:
                conn.close()
                return render_template('user-profile.html', lookavatar=session.get('lookavatar'), error_message="更新失敗，暱稱可能已被使用")
            user_name = name
        ...
        conn.commit()
        conn.close()
        avatar = "static/avatar/" + user_avatar
        session['username'] = user_name
        session['lookavatar'] = avatar

        return render_template('user-profile.html', lookavatar=avatar, success_message="更新成功")

    except Exception as e:
        print(f"An error occurred: {e}")
        return render_template('user-profile.html', lookavatar=session.get('lookavatar'), error_message="更新失敗")
...
if __name__ == '__main__':
    ...
    # 然後啟動 Flask 應用
    app.run('140.131.114.151', port=1024, debug=True)